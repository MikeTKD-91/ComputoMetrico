...
function Sidebar() {
  ...
  const menuItems = [
    { 
      ...
      label: 'Computi', 
      icon: FolderOpen,
      badge: state.computi.length
    },
    { 
      id: 'prezzario', 
      label: 'Prezzario', 
      icon: Database,
      badge: state.prezzario.length
    },
  ] as const;
  ...
  return (
    <aside 
      className={`
        fixed left-0 top-0 h-full bg-white border-r z-40
        ...
        {menuItems.map(item => (
          <button
            ...
            className={`
              ...
              ${state.ui.activeTab === item.id 
                ? 'bg-blue-50 text-blue-700' 
                : 'hover:bg-gray-100 text-gray-700'
              }
              ${!state.ui.sidebarOpen && 'justify-center'}
            `}
          >
        ...
        <div className="px-4 py-2">
          <div className="bg-blue-50 rounded-lg p-3">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {state.computoCorrente.righe.length} righe
            </p>
            <p className="text-lg font-bold text-blue-700 mt-2">
              {new Intl.NumberFormat('it-IT', {
                style: 'currency',
                currency: 'EUR',
              }).format(totaleGenerale)}
            </p>
          </div>
        </div>
      )}
      ...
      {state.ui.sidebarOpen && (
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t">
          <p className="text-xs text-gray-400 text-center">
            Computo Metrico Estimativo v1.0
          </p>
        </div>
      )}
    </aside>
...
function Header() {
  ...
  const handleSalva = () => {
    dispatch({ type: 'SAVE_COMPUTO' });
    alert('Computo salvato!');
  };

  const handleChiudi = () => {
    dispatch({ type: 'CLOSE_COMPUTO' });
  };
    ...
    <header className="h-16 bg-white border-b flex items-center justify-between px-6 sticky top-0 z-30">
      <div className="flex items-center gap-4">
        {state.computoCorrente && (
          <>
            <Button variant="outline" size="sm" onClick={handleChiudi}>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Modificato: {new Date(state.computoCorrente.dataModifica).toLocaleString('it-IT')}
              </p>
            </div>
          </>
        )}
      ...
      <div className="flex items-center gap-2">
        {state.computoCorrente && (
          <>
            <Button variant="outline" size="sm" onClick={handleSalva}>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-500">
            Importa il prezzario da CSV per utilizzarlo nei computi. 
            Le voci del prezzario possono essere ricercate durante la compilazione del computo.
          </p>
        </div>
        <ImportaPrezzario />
      </div>
